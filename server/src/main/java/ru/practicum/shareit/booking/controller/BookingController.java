...
public class BookingController {
    ...
    public BookingDto patchBooking(@RequestParam boolean approved,
    ...
    @GetMapping
    public List<BookingDto> getBookingsForBookerId(@RequestHeader(HEADER_USER_ID) Long userId,
                                                   @RequestParam(defaultValue = "ALL") State state) {
        return bookingService.getBookingsForBookerIdAndState(userId, state);
    }

    @GetMapping("/owner")
    public List<BookingDto> getBookingsForOwnerId(@RequestHeader(HEADER_USER_ID) Long userId,
                                                  @RequestParam(defaultValue = "ALL") State state) {
        return bookingService.getBookingsByOwnerIdAndState(userId, state);
    }

    @GetMapping("/{bookingId}")
    public BookingDto getBookingsByBookerOrOwnerItem(@RequestHeader(HEADER_USER_ID) Long userId,
                                                     @PathVariable Long bookingId) {
