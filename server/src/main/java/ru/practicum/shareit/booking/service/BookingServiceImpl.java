...
public class BookingServiceImpl implements BookingService {
    ...
    @Transactional
    @Override
    public BookingDto addBooking(Long userId, CreateBookingDto createdBookingDto) {
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new NotFoundException("Пользователь не найден"));
        Item item = itemRepository.findById(createdBookingDto.getItemId())
        ...
        List<Booking> bookings = bookingRepository.findAllByIntersectingStartAndEnd(
                item.getId(), createdBookingDto.getStart(), createdBookingDto.getEnd());
        if (!bookings.isEmpty()) {
            throw new ValidationException("Невозможно забронировать");
        }
        Booking booking = BookingMapper.toBookingNew(createdBookingDto, item, user);
